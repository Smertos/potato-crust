...
impl Plugin for GameAssetsPlugin {
    fn build(&self, app: &mut App) {
       app
           .insert_resource(BlockTextureRegistry::default())
...
pub fn load_block_textures(
    ...
    for name in names {
        ...
        let material_handle: Handle<BlockMaterial> = block_materials.add(image_handle.clone().into());
        ...
        if let Err(err) = block_texture_registry.0.insert(name.to_string(), block_texture_handle) {
